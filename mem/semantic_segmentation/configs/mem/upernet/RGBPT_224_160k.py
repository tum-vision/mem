...
# --------------------------------------------------------'
# recommand use this config for models which are self-supervised pretrained and then intermediate fine-tuned on imagenet
_base_ = [
...
model = dict(
    ...
    backbone=dict(
        ...
        mlp_ratio=4,
        qkv_bias=True, # default is True, modelling_finetune and modelling_pretrain also use True
        use_abs_pos_emb=True, # True for RGB-PT
        use_rel_pos_bias=False, # False for RGB-PT
        init_values=0.1,
        drop_path_rate=drop,
